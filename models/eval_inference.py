...
train_loader, val_loader = VRDDataLoader.splits(train, test, mode='rel',batch_size=2, num_workers=1, num_gpus=1)

detector = TransformerModel(classes=VG_ind_to_classes, rel_classes=VG_ind_to_predicates, config = conf,
...
if os.path.basename(conf.ckpt).split('-')[0] == 'vgrel':
