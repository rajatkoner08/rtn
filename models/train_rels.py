...
writer = SummaryWriter(comment='_run#'+str(conf.num_run))
...
for n, param in detector.named_parameters():
    if n.startswith('detector') and  not conf.train_detector:
...
def get_optim(lr):
    ...
    if conf.use_warmup:
        scheduler_slr = StepLR(optimizer, 13, 0.1)
        scheduler = GradualWarmupScheduler(optim, multiplier=1, total_epoch=5, after_scheduler=scheduler_slr)
    else:
...
if len(os.path.basename(conf.ckpt).split('-')) == 2 and not conf.dataset=='gqa':  #todo set start epoch
...
optimizer, scheduler = get_optim(conf.lr * conf.num_gpus ) #* conf.batch_size as it increases lr for large batch size
#save best two
best_epochs = 0 #{}
best_epoch_no = 0
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
    ...
    if best_epochs == 0 or best_epochs <= mAp20:
        ...
        if conf.num_run !=0:
            save_dir = os.path.join(save_dir, 'run' + str(conf.num_run))
    ...
    if not conf.use_warmup:
        scheduler.step(mAp)
writer.close()
#now run evaluation on test
for eval_mode in('predcls','sgcls'):
    eval_rels(conf,run_mode=eval_mode, run_test=True, run_num=conf.num_run, best_epoch=best_epoch_no)
