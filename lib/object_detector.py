...
class ObjectDetector(nn.Module):
    ...
    def rpn_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None,
                  gt_obj_comb=None, train_anchor_inds=None, proposals=None):
        """
        ...
        return all_rois, labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels, gt_obj_comb

    def gt_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None,  gt_obj_comb=None,
        ...
        if gt_rels is not None and self.training:
            rois, labels, rel_labels, obj_comb = proposal_assignments_gtbox(
                rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset, gt_obj_comb.data,
                fg_thresh=0.5)
        else:
            labels = gt_classes[:, 1]
            rel_labels = None
        ...
        return rois, labels, None, None, None, rel_labels, obj_comb

    def proposal_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None, gt_obj_comb=None,
                       train_anchor_inds=None, proposals=None):
        """
        Gets boxes from the RPN
        :param fmap:
        ...
        if self.training:
            all_rois, labels, bbox_targets, obj_comb = proposal_assignments_det(
                rois, gt_boxes.data, gt_classes.data, image_offset, gt_obj_comb.data, fg_thresh=0.5)
        ...
        return all_rois, labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels, obj_comb
    ...
    def forward(self, x, im_sizes, image_offset,
                gt_boxes=None, gt_classes=None, gt_rels=None, proposals=None, gt_obj_comb=None, train_anchor_inds=None,
                return_fmap=False):
        """
        ...
        # Get boxes from RPN
        if self.mode!='gtbox':
            rois, obj_labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels, obj_comb = \
                self.get_boxes(fmap, im_sizes, image_offset, gt_boxes,
                               gt_classes, gt_rels, gt_obj_comb, train_anchor_inds, proposals=proposals)
        else:
            rois = gt_boxes
            obj_labels = gt_classes[:,1]
            bbox_targets = gt_boxes #todo usewd anywhere?
            rpn_scores = None
            rpn_box_deltas =None
            rel_labels = None
        ...
        if (not self.training and not self.mode == 'gtbox') or self.mode in ('proposals', 'refinerels'):
            ...
            if self.training and not self.mode == 'gtbox':
                ...
                pred_to_gtbox[im_inds.data[:, None] != gt_classes.data[None, :, 0]] = 0.0  #set zero to other img box which hv overlap
                ...
                max_seq = torch.max(im_inds.bincount())
            else:
                rm_obj_labels = None
        else:
...
def filter_det(scores, boxes, start_ind=0, max_per_img=100, thresh=0.001, pre_nms_topn=6000,
    ...
    for c_i in valid_cls.squeeze(1).cpu():
        ...
        keep = apply_nms(boxes_ci, scores_ci,
