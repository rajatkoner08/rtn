...
class DecoderRNN(torch.nn.Module):
    def __init__(self, classes, embed_dim, inputs_dim, hidden_dim, recurrent_dropout_probability=0.2,
                 use_highway=True, use_input_projection_bias=True, use_previous_states = False):
        """
        Initializes the RNN
        ...
        self.nms_thresh = 0.3
        self.use_previous_states = use_previous_states
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        previous_embed = self.obj_embed.weight[0, None].expand(batch_size, 100)   #todo why its 0???
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            pred_dist = self.out(previous_state)   #todo part of eq no 4
            out_dists.append(pred_dist)
            if self.use_previous_states:
                hidden_states.append(previous_state)
        ...
        if self.use_previous_states:
            return torch.cat(out_dists, 0), out_commitments, hidden_states
        else:
            return torch.cat(out_dists, 0), out_commitments, None
