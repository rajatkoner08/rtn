...
import numpy as np
import lib.transformer.Constants as Constants
from lib.transformer.Layers import EncoderLayer, DecoderLayer, RelLayer
...
class Encoder(nn.Module):
    ...
    def __init__(
            self,
            n_src_vocab, max_seq, d_word_vec,
            n_layers, n_head, d_k, d_v,
            d_model, d_inner, custom_pe, dropout=0.1):
        ...
        self.pos_index_enc = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(max_seq, d_word_vec//2 if custom_pe else d_word_vec, padding_idx=0),
    ...
    def forward(self, img_feats, src_seq,  src_pos = None, return_attns=False):
        ...
        if src_pos is None:
            ...
            enc_output = img_feats + self.pos_index_enc(src_pos).repeat(1,1,2)
        else:
            enc_output = img_feats +self.pos_index_enc(src_pos)
...
class Decoder(nn.Module):
    ...
    def __init__(
            self,
            n_tgt_vocab, max_seq, d_word_vec,
            n_layers, n_head, d_k, d_v,
            d_model, d_inner, custom_pe, e2e, dropout=0.1):
        ...
        self.pos_index_enc = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(max_seq, d_word_vec//2 if custom_pe else d_word_vec, padding_idx=0),
            freeze=True)  #For edge tow object pos will get encoded, so dim//2

        self.layer_stack = nn.ModuleList([
            DecoderLayer(d_model, d_inner, n_head, d_k, d_v, e2e, dropout=dropout)
            for _ in range(n_layers)])

    def forward(self, obj_comb, tgt_seq, src_seq, enc_output, pos_enc=None, return_attns=False, e2e=True):
        ...
        if pos_enc is None:
            ...
            combined_indices = obj_comb.new_zeros(obj_comb.size())  # todo seq sud start with zero not 1, then how object com will happen  , check content
            combined_indices[:, :, 0::4] = self.pos_index_enc(pos_enc[:, :, 0])[:, :, 0::2]
            combined_indices[:, :, 1::4] = self.pos_index_enc(pos_enc[:, :, 0])[:, :, 1::2]
            combined_indices[:, :, 2::4] = self.pos_index_enc(pos_enc[:, :, 1])[:, :, 0::2]
            combined_indices[:, :, 3::4] = self.pos_index_enc(pos_enc[:, :, 1])[:, :, 1::2]
            dec_output = obj_comb + combined_indices
        else:
            dec_output = obj_comb + self.pos_index_enc(pos_enc)
