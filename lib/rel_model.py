...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
                 ...
                 pass_in_obj_feats_to_edge=True, use_temporal_conn=True):
        ...
        # manage states and objects for temporal connection
        self.use_temporal_conn = use_temporal_conn
        self.hidden_states = []
        self.tid_to_states = {}
        ...
        if self.nl_obj > 0:
            ...
            self.decoder_rnn = DecoderRNN(self.classes, embed_dim=self.embed_dim,
                                          ...
                                          recurrent_dropout_probability=dropout_rate, use_previous_states=True)
        else:
    ...
    def edge_ctx(self, obj_feats, obj_dists, im_inds, obj_preds, box_priors=None):
        ...
        confidence = F.softmax(obj_dists, dim=1).data.view(-1)[  # todo is it eq no
        ...
        edge_reps = self.edge_ctx_rnn(edge_input_packed)[0][0]  # todo eq 5
    ...
    def obj_ctx(self, obj_feats, obj_dists, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None,
        ...
        if self.mode != 'predcls':
            decoder_inp = PackedSequence(
                ...
                ls_transposed)
            obj_dists, obj_preds, self.hidden_states = self.decoder_rnn(
            ...
            # pass sates to next frame for temporal secenario
            # todo probaly this block will be used for both predcls and sgcls, and remove old one
            if self.use_temporal_conn:
                current_tracks = []
                for hs_i, states in enumerate(self.hidden_states):
                    # check if track exists in dict then update else insert
                    current_track = track_id[hs_i, :].cpu().data.numpy()[1]
                    current_tracks.append(current_track)
                    if current_track in self.tid_to_states.keys():
                        self.tid_to_states[current_track] = states
                    else:
                        self.tid_to_states.update({current_track: states})
                # remove old track_id from dict
                for key in self.tid_to_states.copy().keys():
                    if key not in current_tracks:
                        del self.tid_to_states[key]
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None,
        ...
        obj_pre_rep = torch.cat((obj_fmaps, obj_embed, pos_embed), 1)  # todo eq 2 features
...
class RelModel(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', num_gpus=1, use_vision=True, require_overlap_det=True,
        ...
        ###################################
        self.post_lstm = nn.Linear(self.hidden_dim, self.pooling_dim * 2)  # todo not an lstm
    ...
    def forward(self, x, im_sizes, image_offset,
                gt_boxes=None, gt_classes=None, gt_tids=None, gt_rels=None, proposals=None, train_anchor_inds=None,
                return_fmap=False):
        """
        ...
        """
        result = self.detector(x, im_sizes, image_offset, gt_boxes, gt_tids, gt_classes, gt_rels, proposals,
        ...
        track_ids = gt_tids  # todo decide the right appraoch, not valied for SGDet
        ...
        # todo also computes both object and edge context
        result.rm_obj_dists, result.obj_preds, edge_ctx = self.context(
            ...
            boxes.data, result.boxes_all, track_ids)

        # todo eq 3 from paper, probably its 6
        if edge_ctx is None:
        ...
        if self.use_vision:  # todo eq no 6 from paper
