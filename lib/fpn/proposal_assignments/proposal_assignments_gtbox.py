...
@to_variable
def proposal_assignments_gtbox(rois, gt_boxes, gt_classes, gt_rels, image_offset, obj_comb, fg_thresh=0.5):
    """
    ...
    if obj_comb is not None:
        for i, s, e in enumerate_by_image(obj_comb[:, 0]):
            obj_comb[s:e, 1:] += offset[i]
    ...
    return rois, labels, rel_labels, obj_comb
